...
from groundingdino.config.configs.common.data.odinw.mapping_classes import ODINW_13_FILE_MAPPING, ODINW_OVERLAPPED_FILE_MAPPING, OW_ORDER
import re
        ...
        with EventStorage(start_iter) as self.storage:
            try:
                ...
                for self.iter in range(start_iter, max_iter):
                    if self.iter == int(max_iter/2): # int(max_iter/2):
...
def main(args):
    ...
    tm = TaskMemory()
    tm.output_dir = args.output_dir
    task_num = 13

    if args.shot > 0:
        base_dir = f'test/test_odinw13_{args.shot}shot/for_train'
        ow_config_files = [
        'test_pothole.py',
        'test_Egohands_generic.py',
        'test_VehiclesOpenImages.py',
        'test_thermalDogsAndPeople.py',
        'test_aquarium.py',
        'test_pistols.py',
        'test_Raccoon.py',
        'test_PascalVoc.py',
        'test_NorthAmericaMushrooms.py',
        'test_CottontailRabbits.py',
        'test_ShellfishOpenImages.py',
        'test_Packages.py',
        'test_AerialMaritimeDrone_tiled.py'
        ]
        ow_config_files = [ os.path.join(base_dir,ow_config_file) for ow_config_file in ow_config_files]
    else:
        ow_config_files = [
        'test/test_odinw13_fix/for_train/test_pothole.py',
        'test/test_odinw13_fix/for_train/test_Egohands_generic.py',
        'test/test_odinw13_fix/for_train/test_VehiclesOpenImages.py',
        'test/test_odinw13_fix/for_train/test_thermalDogsAndPeople.py',
        'test/test_odinw13_fix/for_train/test_aquarium.py',
        'test/test_odinw13_fix/for_train/test_pistols.py',
        'test/test_odinw13_fix/for_train/test_Raccoon.py',
        'test/test_odinw13_fix/for_train/test_PascalVoc.py',
        'test/test_odinw13_fix/for_train/test_NorthAmericaMushrooms.py',
        'test/test_odinw13_fix/for_train/test_CottontailRabbits.py',
        'test/test_odinw13_fix/for_train/test_ShellfishOpenImages.py',
        'test/test_odinw13_fix/for_train/test_Packages.py',
        'test/test_odinw13_fix/for_train/test_AerialMaritimeDrone_tiled.py'
        ]
    ...
    ow_config_files = ow_config_files[:task_num] if task_num > 0 else []
    
    load_tm = False
    if load_tm:
        tm_save_path = "./output/random_words_random_learned/Packages_task_memory.pt"
        tm = TaskMemory()
        ckpt = torch.load(tm_save_path)
        tm.__dict__.update(ckpt)
        print("load successfully")

    if not args.eval_only:
        for task_num, ow_config_file in enumerate(ow_config_files):
            if task_num < 12 and load_tm:
                continue
            match = re.search(r'for_train/test_(.+)\.py$', ow_config_file)
            ...
            model = do_train(args, cfg)

            stream_dir = os.path.join(args.output_dir, "features")
            os.makedirs(stream_dir, exist_ok=True)
            tm.save_streaming_lda_statistics(stream_dir, suffix=f'_rank{comm.get_rank()}')
            comm.synchronize()
            if comm.is_main_process():
                tm.merge_distributed_lda_statistics(stream_dir, comm.get_world_size())
            comm.synchronize()
    ...
    odinw_config_file ="test/test_odinw13/for_train/test_merge.py"
    if os.path.exists(odinw_config_file):
        ow_config_files += [odinw_config_file]
    ...
    for ow_config_file in ow_config_files:
        ...
        with open(json_path, "w") as jf:
            json.dump(res, jf)
        if 'merge' in ow_config_file:
            classes = [key for key in res['bbox'].keys() if '-' in key]
            classes_ap = {key.split("-", 1)[1]:res['bbox'][key] for key in classes}
            task_ap = {}
            for task_name, task_classes in TaskMemory().task_mapping.items():
                if 'coco' in task_name:
                    continue
                task_ap[task_name] = 0.0
                for c in task_classes.values():
                    assert c.lower() in classes_ap, f"{c} not in classes_ap"
                    task_ap[task_name] += classes_ap[c.lower()]
                task_ap[task_name] /= len(task_classes)
            logger.info(f"AP for each task: {task_ap}")
            average_task = sum(task_ap.values()) / len(task_ap)
            logger.info(f"average task AP:{average_task}")


    # torch.save(TaskMemory().min_scores, "./weights/analyse/class_scores_upper.pt")
    ...
    for k, v in avg_res.items():
        if k != coco_config_file and k != 'test/test_odinw13/for_train/test_merge.py':
            sum_ += v
        else:
            coco_count += 1
    ...
    if 'test/test_odinw13/for_train/test_merge.py' in ow_config_files:
        logger.info(f"AP on OdinW13: {average_task}")
        for task_name in OW_ORDER:
            if task_name in task_ap:
                logger.info(f"AP on {task_name}: {task_ap[task_name]}")
        

    end_time = datetime.datetime.now()
    elapsed_time = end_time - start_time
    logger.info(f"Elapsed time: {elapsed_time}")
        # all_result[temperature] = (avg_res[coco_config_file], average_task, task_ap)
        # torch.save(all_result, "./weights/all_result_mix_add.pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seed", type=int, default=0, required=False, help="path to model checkpoint file")
